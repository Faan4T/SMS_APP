					...
					<div class="card">
						<div class="header">
							<h4 class="title">Trivia Campagins
								<input type="button" class="btn btn-primary" value="Add New" id="btn_right" onclick="window.location='add_trivia.php'" />
							</h4>
							<p class="category">Your already saved list of campaigns.</p>
							<div id="alertArea"></div>
						</div>
                        <?php
                        $sql_trivia = mysqli_query($link,sprintf("select * from campaigns where user_id=%s and type='3' ",
                                                        mysqli_real_escape_string($link,filterVar($_SESSION['user_id'])))
                                            );
                        if(isset($_REQUEST['search']) && $_REQUEST['search'] != ''){
                            $sql_trivia = mysqli_query($link,"SELECT * FROM `campaigns` WHERE `title` LIKE '%".$_REQUEST['search']."%' and user_id=".$_SESSION['user_id']." and type=3") or die(mysqli_error($link));
                        }
                        ?>
                        <div class="col-md-4"><span class="badge badge-success"><?php echo 'Total : '.mysqli_num_rows($sql_trivia); ?></span></div><br>
                        <div class="content table-responsive table-full-width">
                            <div class="row">
                                <form action="trivias.php" class="view_subscriber_class">
                                    <div class="col-md-6"></div>
                                    <div class="col-md-5">
                                        <input type="text" name="search" id="search" class="form-control" placeholder="Search by title" value="<?php echo DBout($_REQUEST['search'])?>" />
                                    </div>
                                    <div class="col-md-1">
							...
							<table id="campaignTable" class="table table-hover table-striped listTable">
								...
								<tbody>
									...
									$sql = sprintf("select * from campaigns where user_id=%s and type='3' order by id desc",
                                                        mysqli_real_escape_string($link,filterVar($_SESSION['user_id']))
                                            );
                                    if(isset($_REQUEST['search']) && $_REQUEST['search'] != ''){
                                        $sql ="SELECT * FROM `campaigns` WHERE `title` LIKE '%".$_REQUEST['search']."%' and user_id=".$_SESSION['user_id']." and type=3";
                                    }
										...
										if($maxLimit >=  DBout($_SESSION['TOTAL_RECORDS'])){
											$maxLimit =  DBout($_SESSION['TOTAL_RECORDS']);	
										}
									...
									?>
												<tr>
													...
													<td align="center">
														<?php ?>
															<?php echo isMediaExists($row['media']);?>
														<?php
														?>
													</td>
													<td>
														<a href="#copyCampaign" title="Duplicate campaign" id="orange" onclick="getCampaignID('<?php echo DBout($row['id'])?>')" data-toggle="modal">
															<i class="fa fa-copy"></i>
														</a>
														<a href="add_trivia.php?id=<?php echo DBout($row['id']);?>" title="Edit campaign"><i class="fa fa-edit"></i></a>
														<i class="fa fa-remove" id="fa_remove" title="Delete campaign" onclick="deleteCampaign('<?php echo DBout($row['id']);?>')"></i>
													</td>
												</tr>
									<?php			
											}	
...
<div id="blockedNumbersSection" class="modal fade" role="dialog">
	<div class="modal-dialog"> 
		<div class="modal-content">
			...
			<div class="modal-body showBlockedNumbers" id="modal_body">Loading...</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
			</div>
		</div>
			...
			<div class="modal-body buklSMSBody">
				...
				<tr>
					...
					<td align="left">
                        ...
                        <input type="button" value="Post on Twitter" class="btn btn-success" id="margin_top" onclick="PostMessage_tw()" />
                        &nbsp;<img src="images/busy.gif" id="loading">&nbsp;<span id="showresponse"></span>
					...
					<table width="100%" id="">
						...
						<tr>
							...
							<td align="left">
								...
								<input type="button" value="Post on Facebook" class="btn btn-success" id="margin_top"  onclick="PostMessage()" />
								&nbsp;<img src="images/busy.gif" id="loading">&nbsp;<span id="showresponse"></span>
								<input type="hidden" name="hidden_sms_id" id="hidden_sms_id" value="">
							</td>
						</tr>
