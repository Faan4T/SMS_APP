					...
					<div class="card">
						<div class="header">
							<h4 class="title">Campaigns <img src="load9.gif" style="margin-left: 20px; display: none" id="showLoading">
								<input style="float: right!important;" type="button" class="btn btn-primary move-right" value="Add New" onclick="window.location='add_client.php'" />
							</h4>
						...
						$sql_campaign = mysqli_query($link, sprintf(
							"select * from campaigns where user_id=%s and type='6'",
								...
								<div class="table-scroll">
									<table id="campaignTable" class="table table-hover table-striped listTable" style="color: #999;">
										...
										<tbody>
											<?php
											$sql = sprintf(
												"select * from campaigns where user_id=%s and type='6' order by id desc",
												mysqli_real_escape_string($link, filterVar($_SESSION['user_id']))
											);
											if (isset($_REQUEST['search']) && $_REQUEST['search'] != '') {
...
<script>
	...
	function sendNotificationMessage(obj, clientID) {
		if (confirm("Are you sure you want to send notifications now?")) {
			...
			$.post("server.php", {
				"cmd": "send_notification_to_customers",
				clientID: clientID
			}, function(r) {
				$('#showLoading').hide();
				window.location = 'view_clients.php';
