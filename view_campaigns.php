					...
					<div class="card">
						<div class="header">
							<h4 class="title">Keyword Campagins
								<input style="float: right!important;" type="button" class="btn btn-primary" value="Add New" onclick="window.location='add_campaign.php'" />
							</h4>
						...
						$sql_campaign = mysqli_query($link, sprintf(
							"select * from campaigns where user_id=%s and type='1'",
							mysqli_real_escape_string($link, filterVar($_SESSION['user_id']))
						));

						if (isset($_REQUEST['search']) && $_REQUEST['search'] != '') {
							$sql_campaign = mysqli_query($link, "SELECT * FROM `campaigns` WHERE `title` LIKE '%" . $_REQUEST['search'] . "%' and user_id=" . $_SESSION['user_id'] . " and type=1") or die(mysqli_error($link));
						}
						...
						<div class="content table-responsive table-full-width">
							<div class="row">
								<form action="view_campaigns.php" class="view_subscriber_class">
									<div class="col-md-6"></div>
									<div class="col-md-5">
										<input type="text" name="search" id="search" class="form-control" placeholder="Search campaign title" value="<?php echo DBout($_REQUEST['search']) ?>" />
									</div>
									<div class="col-md-1">
								...
								<div class="table-scroll">
									<table id="campaignTable" class="table table-hover table-striped listTable" style="color:#999;">
										...
										<tbody>
											<?php
											$sql = sprintf(
												"select * from campaigns where user_id=%s and type='1' order by id desc",
												mysqli_real_escape_string($link, filterVar($_SESSION['user_id']))
											);
											if (isset($_REQUEST['search']) && $_REQUEST['search'] != '') {
												$sql = "SELECT * FROM `campaigns` WHERE `title` LIKE '%" . $_REQUEST['search'] . "%' and user_id=" . $_SESSION['user_id'] . " and type=1";
											}
											...
											$pagelink 	= DBout("view_campaigns.php?");
											...
											if (mysqli_num_rows($res)) {
												...
												while ($row = mysqli_fetch_assoc($res)) {
													if (($row['attach_mobile_device'] == '1') && ($row['device_id'] != '0' && $row['device_id'] != '')) {
														$deviceinfo = getDeviceInfo($row['device_id']);
														$phoneNumber = $deviceinfo['device_name'];
													} else {
											...
											?>
													<tr>
														...
														<td><?php echo DBout($phoneNumber); ?></td>
														<td align="center">
															<?php
															$sql1 = sprintf(
																"select id from follow_up_msgs where group_id=%s",
														...
														<td align="center">
															...
															if (mysqli_num_rows($exe) == '0') {
																...
																<a href="subscribers_stats.php?group_id=<?php echo DBout(encode($row["id"]), ENT_COMPAT) ?>&searchType=subscribers" title="View details" target="_blank"><?php echo DBout(mysqli_num_rows($exe)) ?></a>
															<?php	} ?>
															/
															<?php
															$sql3 = sprintf(
															...
															if (mysqli_num_rows($exe) == '0') {
																...
																<a href="subscribers_stats.php?group_id=<?php echo DBout(encode($row["id"]), ENT_COMPAT); ?>&searchType=unsubscribers" title="View details" target="_blank"><?php echo DBout(mysqli_num_rows($exe)); ?></a>
															<?php		}
															?>
														</td>
														<td align="center">
														...
														<td class="text-center">
															...
															<a href="edit_campaign.php?id=<?php echo DBout($row['id'], ENT_COMPAT) ?>" title="Edit campaign"><i class="fa fa-edit"></i></a>
...
<script src="scripts/view_campaign.js">
</script>